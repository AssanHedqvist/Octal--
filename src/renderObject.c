...
void initRenderObjects(RenderObject objects[], SDL_Renderer *renderer) 
{
    ...
    objects[1].flags = RENDER_ACTIVE;

    objects[2].order = 2;
    objects[2].texture = IMG_LoadTexture(renderer, "resources/stickman/stickmanGreen.png");
    objects[2].imageExtents = (SDL_Rect){0, 0, 128, 256};
    objects[2].screenExtents = (SDL_Rect){400, 300, 32, 64};
    objects[2].flags = 0;

    objects[3].order = 2;
    objects[3].texture = IMG_LoadTexture(renderer, "resources/stickman/stickmanRed.png");
    objects[3].imageExtents = (SDL_Rect){0, 0, 128, 256};
    objects[3].screenExtents = (SDL_Rect){300, 300, 32, 64};
    objects[3].flags = 0;

    objects[4].order = 2;
    objects[4].texture = IMG_LoadTexture(renderer, "resources/stickman/stickmanCyan.png");
    objects[4].imageExtents = (SDL_Rect){0, 0, 128, 256};
    objects[4].screenExtents = (SDL_Rect){300, 300, 32, 64};
    objects[4].flags = 0;

    objects[5].order = 2;
    objects[5].texture = IMG_LoadTexture(renderer, "resources/stickman/stickmanYellow.png");
...
void render(SDL_Renderer *renderer, RenderObject objects[], int length) 
{
    ...
    while(!doneRendering) 
    {
        ...
        {
            if(flagRenderGet(objects[i].flags, RENDER_ACTIVE) && objects[i].order == currentOrder) 
            {
                SDL_RenderCopyEx(
                    renderer,
                    objects[i].texture,
                    &objects[i].imageExtents,
                    &objects[i].screenExtents,
                    0.0,
                    NULL,
                    flagRenderGet(objects[i].flags,FLIP)
                );
            }   
